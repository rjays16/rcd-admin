<template>
	<div class="content-header" id="abstract">
				...
				<div class="container-fluid py-2">
					<div class="row">
						<div class="col-12 pl-0">
							<a class="btn btn-blue border" data-target="#addFreepaper" data-toggle="modal">
								Add Free Paper Submission
							</a>
						</div>
					</div>
				</div>
			...
			<div class="container-fluid mt-2 bg-white corner-rounded p-3">
				...
				<div class="row">
					<b-table
						...
						:busy="isBusy"
						class="m-2"
						:filter-included-fields="['authors', 'member.user.email']"
						...
						<template v-slot:cell(action)="row">
							...
							<button v-if="adminCapability.can_delete_abstract" class="btn btn-red w-25 py-2 corner-22px opensans w-50 corner-pill mb-2" :class='"btn-delete-abstract-" + row.item.id'
								@click="deleteAbstract(row.item.id)">
								Delete
							</button>
							<button v-if="adminCapability.can_resend_abstract_ty_mail" class="btn btn-green py-2 corner-22px opensans w-50 corner-pill" :class='"btn-resend-email-" + row.item.id'
		...
		<ModalAbstractAddFreePaper ref="addFreepaper"/>
	</div>
</template>
