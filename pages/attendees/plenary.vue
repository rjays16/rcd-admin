...
export default {
  ...
  methods: {
    ...
    getAttendance() {
      this.$store.dispatch("attendance/getAttendanceListPlenary")
      .then(res => {
    ...
    exportData(){
      ...
      this.$store.dispatch("attendance/exportPlenary")
        .then(res => {
          const url = URL.createObjectURL(new Blob([res.data]))
          const link = document.createElement('a')
          link.href = url
          link.setAttribute('download', "Plenary_attendance.xlsx")
  ...
  mounted() {
    this.getAttendance()
    this.setNavbarTitle('Plenary Attendance')
