<template>
	<div class="content-header" id="product-catalogue">
		<div class="container-fluid mt-2">
			<div class="container-fluid py-2">
				<div class="row">
					<div class="col-12 pl-0">
						<a class="btn btn-blue border" :href="`/sponsors/edit/${$route.params.id}/product-catalogue/add`">
			...
			<div class="container-fluid mt-2 bg-white corner-rounded p-3">
				...
				<div class="row">
					<b-table
						...
						:filter="productCatalogueCriteria"
						:responsive="true"
						:busy="isBusy"
						class="m-2"
						:show-empty="rows == 0"
                        ...
                        <template v-slot:cell(action)="row">
							<div class="actions same-size text-center">
								<a class="btn btn-blue mb-2" :href="`/sponsors/edit/${$route.params.id}/product-catalogue/edit/${row.item.id}`">
									<i class="fas fa-edit"></i>
								</a>
								<a class="btn btn-red mb-2" @click="deleteProductCatalogue(row.item.id)">
...
export default {
	...
	data() {
		return {
			...
			currentPage: 1,
			perPage: 10,
			productCatalogueCriteria: '',
            productCataloguesFilterTimeout: null,
			isBusy: true,
		}
	},
    computed: {
	...
	methods: {
		...
		search() {
            this.isBusy = true
            clearTimeout(this.productCatalogueFilterTimeout)
            this.productCatalogueFilterTimeout = setTimeout(() => {
                this.productCatalogueCriteria = this.searchTerm
                this.isBusy = false
            }, 300)
        },
		...
		getProductCatalogues() {
			this.$store.dispatch("sponsor_assets/product-catalogue/getProductCatalogues", this.$route.params.id)
			.then(res => {
                this.productCatalogues = res.data
				this.rows = this.productCatalogues.length
				this.isBusy = false
			})
			.catch(err => {
        ...
        deleteProductCatalogue(id) {
            this.$confirm('This will delete the product catalogue. Continue?', 'Warning', {
				...
				this.$store.dispatch("sponsor_assets/product-catalogue/delete", data)
				.then((res) => {
					this.$message.success(res.data.message)
					this.isBusy = true
					setTimeout(() => {
						this.getProductCatalogues()
					}, 3000);
				})
				.catch((err) => {
					this.$message.error(err)
